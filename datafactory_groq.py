...
"""
مصنع البيانات الذكي لـ "زيكو" – يستخدم GROQ API للمهام العامة،
ويؤجل مادة النحو (Arabic Language Grammar) لـ Gemini API عبر ملف انتظار.
يتعامل مع استنفاد الحصة اليومية دون حلقات لا نهائية، ويحفظ النتائج الجزئية.
"""
...
GROQ_RETRY_ATTEMPTS = config.get("groq", {}).get("retry_attempts", 2)
GROQ_RETRY_DELAY = config.get("groq", {}).get("retry_delay", 5)
...
GEMINI_RETRY_ATTEMPTS = config.get("gemini", {}).get("retry_attempts", 2)
GEMINI_RETRY_DELAY = config.get("gemini", {}).get("retry_delay", 5)

if GEMINI_API_KEY:
    gemini_client = genai.Client(api_key=GEMINI_API_KEY, http_options={'api_version': 'v1'})
else:
    gemini_client = None
    logging.warning("⚠️ GEMINI_API_KEY غير موجود، لن تتم معالجة النحو.")

# عامة
HF_DELAY = config.get("processing", {}).get("hf_delay", 2)
MAX_CONSECUTIVE_QUOTA_ERRORS = config.get("processing", {}).get("max_consecutive_quota_errors", 3)
...
def is_quota_error(exception):
    """تحديد ما إذا كان الخطأ بسبب استنفاد الحصة (rate limit / quota)."""
    error_str = str(exception).lower()
    if isinstance(exception, (groq.APIStatusError, groq.RateLimitError)):
        return True
...
def call_groq(prompt, task_type):
    """استدعاء GROQ مع إعادة محاولة محدودة، والتوقف عند استنفاد الحصة."""
    ...
    for attempt in range(GROQ_RETRY_ATTEMPTS):
        try:
            ...
            if is_quota_error(e):
                ...
                logger.error(f"⚠️ استنفاد الحصة (محاولة {attempt+1})، سيتم التوقف إذا تكرر.")
                if consecutive_quota_errors >= MAX_CONSECUTIVE_QUOTA_ERRORS:
                    logger.critical("❌ تم استنفاد الحصة اليومية. إيقاف المعالجة وحفظ النتائج.")
                    raise QuotaExhaustedError("تم استنفاد الحصة اليومية لـ GROQ") from e
                # إذا لم نصل للحد، ننتظر قليلاً ونعيد المحاولة
                time.sleep(GROQ_RETRY_DELAY * (2 ** attempt))
            else:
    ...
    with open(output_file, "a", encoding="utf-8") as f_out, \
        ...
        for idx, it in enumerate(items):
            try:
                ...
                # نرفع الاستثناء لأعلى لإنهاء المعالجة
                logger.error("⚠️ تم استنفاد الحصة، إنهاء معالجة GROQ.")
                raise
            except Exception as e:
...
def process_gemini_grammar_items(items, output_file, defer=True):
    ...
    if not gemini_client:
        logger.error("❌ Gemini غير مهيأ، لن تتم معالجة النحو.")
        return

    # هنا يمكن إضافة معالجة فورية مماثلة مع نفس آلية استنفاد الحصة
    logger.info("معالجة عينات النحو عبر Gemini (فوري) – لم يتم تفعيلها في هذا الإصدار.")
    # نتركها للتوسع المستقبلي

# -------------------- الدالة الرئيسية --------------------
def main():
    max_samples = config.get("data", {}).get("max_samples", 500)
    defer_grammar = True   # تأجيل النحو إلى ملف انتظار
    ...
    try:
        ...
        logger.critical("❌ تم استنفاد حصة GROQ. تم حفظ ما تم معالجته.")
        # لا نعيد رفع الاستثناء، فقط ننهي المعالجة
    except Exception as e:
        logger.error(f"❌ خطأ غير متوقع في GROQ: {e}")
    ...
    if defer_grammar and grammar_items:
